...
function yora_ensure_schema(mysqli $db): void
{
    ...
    $columnas = [
        'configuracion_web' => [
            ...
            'driver_min_version_code' => 'INT NOT NULL DEFAULT 0',
            'driver_latest_version'   => 'VARCHAR(40) DEFAULT NULL',
...
function yora_require_write(): void
{
    ...
    // App nativa: Authorization Bearer / X-Yora-Token (sin cookie CSRF).
    if (yora_api_token_request() !== '') {
        return;
    }
...
{
    // Al cerrar sesión siempre Offline: no deben quedar en el mapa de flota.
    yora_exec($db, 'UPDATE conductores SET token_app = NULL, en_linea = 0 WHERE id = ?', 'i', $id);
}
...
/**
 * Auth para APK / API: token en Authorization Bearer o X-Yora-Token (= conductores.token_app).
 */
function yora_api_token_request(): string
{
    $hdr = (string) ($_SERVER['HTTP_AUTHORIZATION'] ?? $_SERVER['REDIRECT_HTTP_AUTHORIZATION'] ?? '');
    if (preg_match('/Bearer\s+(\S+)/i', $hdr, $m)) {
        return trim($m[1]);
    }
    return trim((string) ($_SERVER['HTTP_X_YORA_TOKEN'] ?? ''));
}

function yora_require_conductor_api(): int
{
    $token = yora_api_token_request();
    if ($token === '') {
        yora_fail('Sesión inválida', 401);
    }
    $db = $GLOBALS['conexion'] ?? null;
    if (!($db instanceof mysqli)) {
        yora_fail('Error interno', 500);
    }
    $fila = yora_one($db, 'SELECT id FROM conductores WHERE token_app = ? LIMIT 1', 's', $token);
    if (!$fila) {
        yora_fail('Tu cuenta se abrió en otro dispositivo. Vuelve a iniciar sesión.', 401);
    }
    return (int) $fila['id'];
...
/**
 * Abre sesión única y devuelve el token para clientes nativos (sin depender de cookie).
 */
function yora_conductor_abrir_sesion_api(mysqli $db, int $id): string
{
    yora_session_start();
    yora_session_regenerate();
    $token = bin2hex(random_bytes(32));
...
/**
 * Auth unificado: token API (APK) o sesión PHP (web).
 */
function yora_require_conductor_any(): int
{
    if (yora_api_token_request() !== '') {
        return yora_require_conductor_api();
    }
    return yora_require_conductor();
}

/**
 * Columna debe_cambiar_password: cuentas con clave generada por HQ
 * deben elegir una propia al entrar por primera vez.
 */
function yora_pass_flag_esquema(mysqli $db): void
{
    static $listo = false;
    if ($listo) {
        return;
    ...
    try {
        $col = $db->query("SHOW COLUMNS FROM conductores LIKE 'debe_cambiar_password'");
        if ($col && $col->num_rows === 0) {
            $db->query(
                "ALTER TABLE conductores ADD COLUMN debe_cambiar_password TINYINT(1) NOT NULL DEFAULT 0 AFTER password"
            );
        }
    } catch (Throwable $e) {
        error_log('yora_pass_flag_esquema: ' . $e->getMessage());
    }
    $listo = true;
}

function yora_conductor_debe_cambiar_password(array $row): bool
{
    return (int) ($row['debe_cambiar_password'] ?? 0) === 1;
}
...
function yora_admin_paginas(): array
{
    return [
        'super' => [
            ...
            'notificaciones.php', 'banners.php', 'mantenimiento.php', 'ajustes.php', 'directorio.php', 'usuarios.php',
            'verificar_imagenes.php', 'mapa.php',
        ],
        'comercios' => [
            'comercios.php', 'clientes.php', 'comandas.php', 'recargas.php', 'soporte.php', 'directorio.php', 'mapa.php', 'mantenimiento.php',
        ],
        'drivers' => [
            'conductores.php', 'verificaciones.php', 'comandas.php', 'gestion_pagos.php', 'mapa.php', 'banners.php', 'notificaciones.php', 'mantenimiento.php',
        ],
    ];
}
...
function yora_mime_archivo(string $path): string
{
    if ($path === '' || !is_file($path)) {
        return '';
    }
    if (class_exists('finfo')) {
        $m = (string) ((new finfo(FILEINFO_MIME_TYPE))->file($path) ?: '');
        if ($m !== '') {
            return $m;
        }
    }
    ...
    // Magia básica por cabecera (servidores sin fileinfo).
    $fh = @fopen($path, 'rb');
    if ($fh) {
...
function yora_upload(array $file, string $destDir, array $allowedExt = ['jpg', 'jpeg', 'png', 'webp'], int $maxBytes = 5242880, bool $allowPdf = false): ?string
{
    ...
    $mime = yora_mime_archivo((string) $file['tmp_name']);
    $map = [
    ...
    if (!isset($map[$ext]) || !in_array($mime, $map[$ext], true)) {
        ...
        if ($desde_mime && in_array($desde_mime, $allowedExt, true)) {
            ...
            // Extensión conocida y sin MIME (fileinfo apagado): aceptar jpg/png de cámara.
            // ok
        } else {
            return null;
        }
...
/** Extrae "Ref: …" del texto de direccion_entrega (referencia del comercio). */
function yora_texto_referencia(?string $direccion): string
{
    if (!$direccion || stripos($direccion, 'Ref') === false) {
        return '';
    }
    if (preg_match('/\|\s*Ref\s*:\s*([^|]+)/iu', $direccion, $m)) {
        return trim($m[1]);
    }
    return '';
}
...
function yora_phpmailer_cargado(): bool
{
    ...
    $rutas = [
        // Domino app (copia local)
        __DIR__ . '/public_html/PHPMailer/',
        __DIR__ . '/public_html/hq_sistema_interno_v1/PHPMailer/',
        __DIR__ . '/PHPMailer/',
        // aaPanel / wwwroot (ruta real en este VPS)
        '/www/wwwroot/yoradelivery.com/public_html/PHPMailer/',
        '/www/wwwroot/yoradelivery.com/public_html/hq_sistema_interno_v1/PHPMailer/',
        '/www/wwwroot/app.yoradelivery.com/public_html/PHPMailer/',
        // cPanel legado
