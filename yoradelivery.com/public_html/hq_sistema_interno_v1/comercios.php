...
yora_require_admin_pagina('comercios.php');
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aprobar_doc'])) {
    if (!yora_verify_same_origin() && !yora_csrf_ok()) { header('Location: comercios.php'); exit; }
    $id_doc = (int) $_POST['aprobar_doc'];
    yora_exec($conexion, "UPDATE comercios SET estado_documentos = 'Verificado' WHERE id = ?", 'i', $id_doc);
    header("Location: comercios.php?aprobado=1&tab=verificaciones");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['rechazar_doc'])) {
    ...
    yora_exec($conexion, "UPDATE comercios SET estado_documentos = 'Rechazado' WHERE id = ?", 'i', $id_doc);
    header("Location: comercios.php?doc_rechazado=1&tab=verificaciones");
    exit;
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['agregar_restaurante'])) {
    ...
    $check = $telefono === '' ? true : (bool) yora_one($conexion, 'SELECT id FROM comercios WHERE telefono = ?', 's', $telefono);
    if($check) {
        if ($mensaje === '') { $mensaje = "<div class='alert error'>Ese número de teléfono ya está registrado.</div>"; }
    } else {
        $sql = "INSERT INTO comercios (nombre, rif, razon_social, direccion, latitud, longitud, lat, lng, telefono, correo, password, billetera, logo_url, tipo_comercio, estado_documentos, estatus)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aprobar_comercio'])) {
    ...
    $info = yora_one($conexion, 'SELECT nombre, correo, telefono FROM comercios WHERE id = ?', 'i', $id);
    if ($info) {
        $clave = yora_clave_aleatoria();
        $hash = password_hash($clave, PASSWORD_DEFAULT);
        yora_exec($conexion, "UPDATE comercios SET estatus = 'activo', password = ? WHERE id = ?", 'si', $hash, $id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['rechazar_comercio'])) {
    ...
    $info = yora_one($conexion, 'SELECT nombre, correo, telefono FROM comercios WHERE id = ?', 'i', $id);
    if ($info) {
        yora_exec($conexion, "UPDATE comercios SET estatus = 'rechazado' WHERE id = ?", 'i', $id);
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['actualizar_restaurante'])) {
    ...
    try {
        ...
        yora_exec(
            $conexion,
            'UPDATE comercios SET nombre = ?, rif = ?, razon_social = ?, telefono = ?, direccion = ?, latitud = ?, longitud = ?, lat = ?, lng = ?, billetera = billetera + ?, descripcion = ?, categoria = ?, horario = ?, instagram = ?, perfil_publico = ?, gestion_menu = ?, slug = ? WHERE id = ?',
            'sssssdddddssssiisi',
        ...
        try {
            yora_exec(
                $conexion,
                'UPDATE comercios SET correo = ?, hora_abre = NULLIF(?, \'\'), hora_cierra = NULLIF(?, \'\') WHERE id = ?',
                'sssi',
                $correo,
                $abre,
                $cierra,
            ...
            error_log('comercios extra: ' . $e2->getMessage());
        }

        try {
            yora_exec($conexion, 'UPDATE comercios SET tipo_comercio = ? WHERE id = ?', 'si', $tipo_comercio, $id_comercio);
        } catch (Throwable $eTipo) {
            error_log('comercios tipo: ' . $eTipo->getMessage());
        }
        ...
        if ($logo_err === UPLOAD_ERR_OK && (int) ($_FILES['logo']['size'] ?? 0) > 5 * 1024 * 1024) {
            ...
            try {
                $logo = yora_guardar_logo_comercio($_FILES['logo']);
                if (!empty($logo['ok'])) {
                    yora_exec($conexion, 'UPDATE comercios SET logo_url = ? WHERE id = ?', 'si', $logo['url'], $id_comercio);
                } else {
                    $aviso_logo = ' ' . yora_h((string) ($logo['error'] ?? 'El logo no se pudo guardar.'));
                }
            } catch (Throwable $e3) {
                error_log('comercios logo: ' . $e3->getMessage());
        ...
        if(!empty($_POST['password'])) {
            ...
            yora_exec($conexion, 'UPDATE comercios SET password = ? WHERE id = ?', 'si', $hash, $id_comercio);
        }

        $mensaje = "<div class='alert success'>¡Comercio actualizado correctamente!" . $aviso_logo . "</div>";
    } catch (Throwable $e) {
        error_log('comercios actualizar: ' . $e->getMessage());
...
$lista_restaurantes = $conexion->query("
    SELECT r.*, 
    (SELECT COUNT(id) FROM comandas c WHERE c.comercio_id = r.id AND c.estatus = 'Entregado') AS entregas_totales,
    (SELECT COUNT(id) FROM comandas c WHERE c.comercio_id = r.id AND c.estatus = 'Entregado' AND COALESCE(c.fecha_entrega, c.fecha_creacion) >= DATE_FORMAT(NOW(), '%Y-%m-01')) AS club_mes
    FROM comercios r ORDER BY r.id DESC
");
...
<head>
    ...
    <title>Yora Admin | comercios</title>
    ...
    <style>
        ...
        .credito-cell { display:flex; flex-direction:column; gap:4px; min-width:120px; }
        .credito-disp { font-weight:800; font-size:0.95rem; color:#0f766e; }
        .credito-lim { font-size:0.72rem; color:var(--text-muted); }
        .credito-extra { font-size:0.78rem; font-weight:800; color:#16a34a; }
        .credito-bloqueado { color:#dc2626; font-weight:800; font-size:0.72rem; }
                                ...
                                <tr>
                                    ...
                                    <td><?php
                                        ...
                                        $cred = function_exists('yora_credito_estado')
                                            ...
                                            : ['disponible' => 0, 'limite' => 0, 'bloqueado' => false];
                                        $billetera_extra = round((float) ($row['billetera'] ?? 0), 2);
                                    ?>
                                        <div class="credito-cell">
                                            <span class="credito-disp">$<?php echo number_format((float) ($cred['disponible'] ?? 0), 2); ?></span>
                                            <span class="credito-lim">Límite $<?php echo number_format((float) ($cred['limite'] ?? 0), 2); ?></span>
                                            <?php if ($billetera_extra > 0.009): ?>
                                                <span class="credito-extra" title="Saldo prepago histórico sin convertir">+ $<?php echo number_format($billetera_extra, 2); ?> extra</span>
                                            <?php endif; ?>
                                            <?php if (!empty($cred['bloqueado'])): ?>
                                                <span class="credito-bloqueado">Bloqueado</span>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                    <td>
        ...
        <div class="modal-box">
            ...
            <form method="POST" enctype="multipart/form-data"><?php echo yora_csrf_field(); ?>
                ...
                <div class="form-group info-caja" style="background: #fff7ed; border-color: #ffedd5;">
                    <label style="color:var(--yora-orange); font-weight:bold;">Saldo extra histórico ($)</label>
                    <small style="color:var(--text-muted); display:block; margin-bottom:10px;">Prepago viejo (se muestra como “+ extra” en el directorio). No es el límite de crédito. Positivo suma, negativo resta.</small>
