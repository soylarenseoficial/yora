...
function yora_push_conductores(mysqli $db, string $titulo, string $cuerpo, string $url = '/dashboard.php', bool $solo_online = true): int
{
    ...
    // 1) IDs reales de drivers (no el tag en_linea, que casi nadie tenía).
    $ok += yora_onesignal_enviar($db, $externos, $titulo, $cuerpo, $url);
    if ($ok === 0 && $subs_os) {
        $ok += yora_onesignal_enviar_ids($db, $titulo, $cuerpo, $url, 'include_subscription_ids', $subs_os);
    }
    if ($ok === 0 && $players) {
        $ok += yora_onesignal_enviar_ids($db, $titulo, $cuerpo, $url, 'include_player_ids', $players);
    }
    $objetivo = count($filas);
    // 2) Si llegaron pocos (APKs sin login de OneSignal), avisa a todos los suscritos.
    if ($ok === 0 || ($solo_online && $objetivo > 0 && $ok < max(1, (int) floor($objetivo * 0.3)))) {
...
function yora_onesignal_base(array $cfg, string $titulo, string $cuerpo, string $url): array
{
    ...
    return [
        ...
        'priority' => 10,
        'ttl' => 600,
        'existing_android_channel_id' => 'yora-pedidos',
        'android_accent_color' => 'FFE4441B',
        'android_visibility' => 1,
        'android_sound' => 'default',
        'android_led_color' => 'FFE4441B',
        'small_icon' => 'ic_stat_yora',
        'large_icon' => 'https://yoradelivery.com/uploads/Isotipo.png',
        'huawei_sound' => 'default',
        'ios_sound' => 'default',
        'ios_badgeType' => 'Increase',
        'ios_interruption_level' => 'time_sensitive',
        'android_group' => 'yora-pedidos',
